...
public class Min30SecsHttpDataReaderJob implements Job {
	private static Logger logger = Logger.getLogger(Min30SecsHttpDataReaderJob.class);
	...
	public static void main(String[] args) {
		System.out.println("Min30SecsHttpDataReaderJob run...");
		ExecutorService exec = Executors.newFixedThreadPool(10);
		List<Snc01> lstSNC01 = getEnableSNCX01SByMins((String) hm.get("ArduinoDBServerIP"), (String) hmSite.get("site"),"30");
		for (int i = 0; i < lstSNC01.size(); i++) {
			...
			MinXHttpDataReader dr = new MinXHttpDataReader((String) hm.get("ArduinoDBServerIP"), snc.getIp(),
					snc.getMachineName(), false);  // 0.5 秒 K type 不發 email
			exec.execute(dr);
		}
	}

	public void execute(JobExecutionContext context) throws JobExecutionException {
		logger.info("Min30SecsHttpDataReader run...");
		hm = SettingManager.getConfigHM();
		List<Snc01> lstSNC01 = getEnableSNCX01SByMins((String) hm.get("ArduinoDBServerIP"), (String) hmSite.get("site"),"0.5");
		if(lstSNC01.size()==0)
			return;
		ExecutorService exec = Executors.newFixedThreadPool(10);
	...
	private static List<Snc01> getEnableSNCX01SByMins(String strWSServerIP, String strSite, String strMins) {
		...
		if (!"".equals(strWSServerIP) && strWSServerIP !=null) {
